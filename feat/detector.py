    ...
    def __init__(
        ...
        Attributes:
            info (dict):
                ...
                landmark_model (str, default=mobilenet): Name of landmark model
                au_model (str, default=rf): Name of Action Unit detection model
                emotion_model (str, default=resmasknet): Name of emotion detection model.
                ...
                facepose_model_columns (list): Column names for facepose model output
    ...
    def _batch_hog(self, frames, detected_faces, landmarks):
        ...
        for i in range(len(flat_land)):
            ...
            convex_hull, new_lands = self.extract_face(
                frame=frames[frame_assignment], detected_faces=[flat_faces[i][0:4]], landmarks=flat_land[i],
                size_output=112)
            ...
            for i, sessions in enumerate(detected_faces):
                for j, faces in enumerate(sessions):
                    ...
                    facepose_df = pd.DataFrame(
                        poses[i][j], columns=FACET_FACEPOSE_COLUMNS, index=[counter + i]
                    )
